...
import java.util.{ArrayList, Properties, List => JavaList}
...
import org.apache.hadoop.hive.service.HiveServer.{HiveServerCli, HiveServerHandler, ThriftHiveProcessorFactory}
...
import org.apache.thrift.transport.{TServerSocket, TSocket, TTransport, TTransportFactory}
 ...
 */
object SharkServer extends Logging {

  val activeSessions = new  mutable.HashSet[String]
...
class GatedSharkServerHandler(latch:CountDownLatch, remoteClient:String,
    sessionID:String) extends SharkServerHandler with Logging {
  override def execute(cmd: String): Unit = {
    ...
    if (cmd.startsWith("kill ")) {
      ...
      SharkServer.activeSessions.remove(sessionIDToCancel)
    } else {
      // Session ID is used as spark job group
      // Job groups control cleanup/cancelling of unneeded jobs on connection terminations
      SparkSQLEnv.sparkContext.setJobGroup(sessionID, "Session ID = " + sessionID)
      super.execute(cmd)
    }
  }
}
...
class SharkServerHandler extends HiveServerHandler with Logging {
